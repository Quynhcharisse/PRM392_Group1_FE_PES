...
export const getTermList = async () => {
  try {
    const response = await axiosClient.get("/admission/term")
...
export const getDefaultGrade = async (grade) => {
  try {
    const response = await axiosClient.get('/admission/default/fee', {
      params: {grade}
    });
    return response.data;
  } catch (error) {
...
export const getTermYears = async () => {
  try {
    const response = await axiosClient.get('/admission/years');
